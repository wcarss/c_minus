...
int print_identities(HList l);
void add_to_hlist(HList l, int num, int selector);
...
int main()
{
  ...
  while(status != exiting)
  {
    ...
    result += test("remove on empty hlist", 0, 0, selector, got_null);
    result += test("remove on hlist with 1 element", 1, 0, selector, got_null);
...
int test(const char *test_description, int number_of_elements, int index, int add_selector, int expect)
{
  ...
  HList l = hlist_new();
  add_to_hlist(l, number_of_elements, add_selector);
  printf("Test %d, '%s':\n", test_id, test_description);
  print_identities(l);

  /* Set the position within the hlist */
  ...
  /* tear down the hlist and incremement the test count*/
  hlist_kill(l);
  test_id++;
...
void add_to_hlist(HList l, int num, int selector)
{
  ...
  for(i = 0; i < num; i++)
  {
    ...
    add[selector](l, hnode_new(NULL, NULL, NULL, NULL, NULL));
  }
}
